...
---
title: 文章標題
date: 2025-09-29          ← 也是網址（/blog/2025-09-29/），不能和別篇重複
...
- **照片**就是單獨一行的 `![](檔名)`，想換位置就把那一行剪下貼到別的段落之間。照片的檔案放在 `assets/blog/<日期>/`。圖說（alt）可以寫在方括號裡：`![湖邊的早晨](03.webp)`，不寫會自動補。
...
```
python scripts/photo.py blog 2025-09-29 D:\照片\a.jpg D:\照片\b.jpg
