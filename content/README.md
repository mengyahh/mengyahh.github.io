...
cover: 03.webp             ← 封面圖（列表縮圖＋社群分享大圖）；不寫就用第一張照片
summary: 列表上顯示的一兩句簡介
...
- **圖說**寫在檔名後面的引號裡：`![](03.webp "食堂：以前僧侶生活與吃飯的地方")`，會顯示在照片下方。
- **一大堆照片**可以放成縮圖牆，點一下放大、可左右切換：
  ...
  方括號裡的字是這張照片放大時顯示的名稱（可省略）。縮圖用 `22-t.webp`（`photo.py` 會自動做），沒有縮圖檔就直接用原圖。
- 編號清單從哪個數字開始就會顯示哪個（例如中間夾著照片的 `3. …` 會從 3 開始）；`<sub>小字</sub>`、`<br>` 這兩個 HTML 標記可以直接寫。
- **封面圖**：front matter 的 `cover: 03.webp`（照片檔名，放在這篇的照片資料夾裡）。它同時是：部落格列表上的縮圖、Google / Facebook / LINE / Twitter(X) 分享時的大圖卡片、搜尋結果用的圖。想換封面，把檔名改成那一張就好；不寫 `cover:` 的話，預設用文章裡的第一張照片。
...
```
![彥根城天守閣在藍天下](03.webp)          ← 好：說明照片內容
![](03.webp)                              ← 可以：自動補文章標題
![IMG_2043](03.webp)                       ← 不建議：沒有資訊
```

想在照片下方也顯示說明文字，另外加引號：`![彥根城天守閣](03.webp "國寶五城之一")`。
